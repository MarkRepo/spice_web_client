Application = $.spcExtend(wdi.DomainObject, {
    ...
    init: function (c) {
        ...
        this.isInputChannelConnected = false;
        this.setup();
    },
	...
	onChannelConnected: function(params) {
		...
		if (channel === wdi.SpiceVars.SPICE_CHANNEL_INPUTS) {
            this.isInputChannelConnected = true;
    ...
    onClientInput: function (params) {
        var data = params;
		var type = data[0];
        if(this.isInputChannelConnected){
            this.inputProcess.send(data, type);
        }
        else{
            this.clientGui.ack_wait = 0;
        } 
	},
        ...
        switch (action) {
            ...
            case 'getIDR':
                var packet = new wdi.SpiceDataHeader({
                        type: wdi.SpiceVars.SPICE_MSGC_DISPLAY_IDR, 
                        size: 4
                }).marshall();
                var id = 0;
                var body = wdi.SpiceObject.numberTo32(id);
                packet = packet.concat(body);
                this.spiceConnection.channels[wdi.SpiceVars.SPICE_CHANNEL_DISPLAY].send(packet);     
		} 
    },
