wdi.PlaybackProcess = $.spcExtend(wdi.EventObject.prototype, {
	...
	process: function(spiceMessage) {
		...
		if (this.hasAudioSupport) {
			//console.log("playback messageType: " + spiceMessage.messageType);
			switch (spiceMessage.messageType) {
				...
				case wdi.SpiceVars.SPICE_MSG_PLAYBACK_VOLUME:
					var packet = spiceMessage.args;
					//console.log("playback volumes[0]: " + packet.volumes[0] + ", volumes[1]: " + packet.volumes[1]);
					if(packet.volumes[0]<=30840){
						this.player.setVolume(packet.volumes[0]/30840/10);
					}else if(packet.volumes[0]<=36494){
						this.player.setVolume((packet.volumes[0]-30840)/(36494-30840)/10 + 0.1);
					}else if(packet.volumes[0]<=39578){
						this.player.setVolume((packet.volumes[0]-36494)/(39578-36494)/10 + 0.2);
					}else if(packet.volumes[0]<=41891){
						this.player.setVolume((packet.volumes[0]-39578)/(41891-39578)/10 + 0.3);
					}else if(packet.volumes[0]<=43433){
						this.player.setVolume((packet.volumes[0]-41891)/(43433-41891)/10 + 0.4);
					}else if(packet.volumes[0]<=44975){
						this.player.setVolume((packet.volumes[0]-43433)/(44975-43433)/10 + 0.5);
					}else if(packet.volumes[0]<=45746){
						this.player.setVolume((packet.volumes[0]-44975)/(45746-44975)/10 + 0.6);
					}else if(packet.volumes[0]<=47288){
						this.player.setVolume((packet.volumes[0]-45746)/(47288-45746)/10 + 0.7);
					}else if(packet.volumes[0]<=48059){
						this.player.setVolume((packet.volumes[0]-47288)/(48059-47288)/10 + 0.8);
					}else{
						this.player.setVolume((packet.volumes[0]-48059)/(48830-48059)/10 + 0.9);
					}
					//this.player.setVolume(packet.volumes[0]/48830);
					break;
				case wdi.SpiceVars.SPICE_MSG_PLAYBACK_MUTE:
					var packet = spiceMessage.args;
					//console.log("playback Mute: " + packet.mute);
					break;
